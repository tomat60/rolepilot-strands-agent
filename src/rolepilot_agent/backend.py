...
class MemoryBackend:
    ...
    def __init__(self) -> None:
        self.opportunities = {
            1: Opportunity(id=1, title="Lifestyle Campaign - Urban Commuter", brand="Northline Studio", required_assets=("portrait", "full_body", "english_intro", "showreel"), raw_text="Male talent 28-38. Warsaw. Existing approved materials accepted."),
            2: Opportunity(id=2, title="Fintech Product Film - Office Role", brand="Orbit Casting", required_assets=("portrait", "slate", "self_tape"), requires_new_recording=True, raw_text="New slate and improvised self-tape required."),
            3: Opportunity(id=3, title="Travel App - Presenter", brand="Frame & Form", required_assets=("portrait", "showreel"), manual_review_required=True, raw_text="Usage details require manual review before consent."),
        }
        self.assets = [Asset(1, "portrait", "Portrait 01"), Asset(2, "full_body", "Full body"), Asset(3, "english_intro", "English intro"), Asset(4, "showreel", "Showreel")]
    ...
    def create_run(self, opportunity_id: int) -> dict:
        ...
        run = ApplicationRun(id=self._next_run_id, opportunity_id=opportunity_id, readiness=94, audit_events=["Opportunity requirements normalized", "Approved materials matched", "Deterministic safety gate passed", "Preparation state persisted", "Human approval required before external action"])
    ...
    def record_human_decision(self, run_id: int, approved: bool) -> dict:
        ...
        run.audit_events.append("Human approval recorded" if approved else "Human changes requested")
...
class XanoBackend:
    ...
    @classmethod
    def _normalize_run_response(cls, response, *, expected_opportunity_id: int | None = None, expected_run_id: int | None = None, approved: bool | None = None) -> dict:
        ...
        if expected_opportunity_id is not None:
            ...
            opportunity_id = cls._safe_int(raw_run.get("opportunity_id"), "opportunity id")
            ...
            opportunity_id = cls._safe_int(raw_opportunity_id, "opportunity id") if raw_opportunity_id is not None else None
        if approved is None:
            approval_state = "PENDING_HUMAN_APPROVAL"
            audit_events = ["Deterministic safety gate passed", "Xano preparation state persisted", "Human approval required before external action"]
        else:
            approval_state = "APPROVED_DEMO_STATE" if approved else "CHANGES_REQUESTED"
            audit_events = ["Human approval recorded" if approved else "Human changes requested", "External submission remains disabled"]
        return {"id": run_id, "opportunity_id": opportunity_id, "readiness": 0, "approval_state": approval_state, "audit_events": audit_events, "external_submission_performed": False}
    ...
    def list_opportunities(self) -> list[dict]:
        ...
        for item in response:
            ...
            normalized.append({"id": self._safe_int(item.get("id"), "opportunity id"), "title": self._safe_display_text(item.get("title"), "opportunity title"), "brand": self._safe_display_text(item.get("brand"), "opportunity brand")})
        return normalized
    ...
    def analyze(self, opportunity_id: int) -> dict:
        ...
        if "opportunity_id" not in result:
            raise RuntimeError("Xano returned malformed opportunity id")
        remote_opportunity_id = self._safe_int(result.get("opportunity_id"), "opportunity id")
        if remote_opportunity_id != opportunity_id:
            raise RuntimeError("Xano returned inconsistent opportunity id")
        safety_warning: str | None = None
        raw_readiness = result.get("readiness", 0)
        if isinstance(raw_readiness, bool) or not isinstance(raw_readiness, int):
            readiness = 0
        ...
        normalized = {"opportunity_id": opportunity_id, "readiness": readiness, "state": state, "can_prepare": can_prepare, "reasons": reasons}
        if safety_warning is not None:
    ...
    def create_run(self, opportunity_id: int) -> dict:
        ...
        if not analysis.get("can_prepare") or analysis.get("state") != "READY":
            raise SafetyGateError(f"Opportunity {opportunity_id} cannot be prepared: {analysis.get('state')}")
        response = self._request("POST", "/runs", {"opportunity_id": opportunity_id})
        return self._normalize_run_response(response, expected_opportunity_id=opportunity_id)

    def record_human_decision(self, run_id: int, approved: bool) -> dict:
        response = self._request("POST", f"/runs/{run_id}/approval", {"approved": approved})
        return self._normalize_run_response(response, expected_run_id=run_id, approved=approved)
