...
## Active milestone
M0-M3 acceptance recovery, bounded M4 reliability, and safe M5 live-path preparation on the same PR while GitHub Actions startup is unavailable.
...
- M5 preparation now includes an explicit Bedrock live-model cost/configuration gate. The normal CLI will not invoke a model unless `--live-bedrock` is supplied.
...
- Previous exact head `1a7227163812f2439e2c6fd3f31af096ea3d041f` produced CI run `33834813054`; both matrix jobs had `runner_id=0`, `steps=[]`, and ended before checkout, so this remains an infrastructure startup failure rather than executed product/test evidence.
- A fresh container clean-check on 2026-09-04 again failed before clone because the runtime could not resolve `github.com`; this does not satisfy acceptance and is not product failure evidence.
- Current Bedrock-gate implementation/docs head before this state-only commit is `ab0f22c9a2c4cea21d432d52e0e25bd1356177aa`.
...
## Evidence still required before accepting M0-M5-prep slice
...
1. Obtain executed clean-environment verification for M0-M5-prep as soon as infrastructure permits.
...
4. Keep M6 submission assets moving before the final competition days.
